...
class DeviceRepository: Repository {
  ...
  func getAll() -> [Device] {
    ...
    let devices_MO = (try? CoreDataManager.backgroundContext.fetch(fetchRequest)) as! [Device_ManagedObjects]
    
    var devices: [Device] = []
    
      for device_MO in devices_MO {
          let device = Device(id: device_MO.id!,
                            name: device_MO.name!,
                            rssi: nil, isTracking: device_MO.isTracking, givenName: device_MO.givenName, location: Location(latitude: device_MO.latitude, longitude: device_MO.longitude))
  ...
  func get(identifier: Int) -> Device? {
    return Device(id: UUID(uuidString: "68847190-2FD3-8936-66C0-CB6D182176E7")!, name: "123f", rssi: 123, isTracking: nil, givenName: nil, location: nil)
  }
  ...
  func create(a: Device) -> Bool {
    ...
    let device_ManagedObjectArr = (try? CoreDataManager.backgroundContext.fetch(fetchRequest)) as! [Device_ManagedObjects]
    ...
    let device_ManagedObject = NSEntityDescription.insertNewObject(forEntityName: "Device", into: CoreDataManager.backgroundContext) as! Device_ManagedObjects
